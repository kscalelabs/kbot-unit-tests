...
async def wave_sequence(kos: KOS) -> None:
    ...
    await move_to_target(kos, 0.0, -70.0, ACCEL, V_MAX, dt, 22)  # shoulder roll up
    await move_to_target(kos, 0.0, -20.0, ACCEL, V_MAX, dt, 21)  # shoulder pitch up
    await move_hands(kos, right_hand_ids, 1)
    await move_to_target(kos, 0.0, -80.0, ACCEL, V_MAX, dt, 23)
    await move_to_target(kos, 0.0, 125.0, ACCEL, V_MAX, dt, 24)
    for _ in range(3):
        await move_to_target(kos, 125.0, 90.0, ACCEL, V_MAX, dt, 24)
        await move_to_target(kos, 90.0, 125.0, ACCEL, V_MAX, dt, 24)
    await move_to_target(kos, 125.0, 0.0, ACCEL, V_MAX, dt, 24)
    await move_to_target(kos, -80.0, 0.0, ACCEL, V_MAX, dt, 23)
    await move_hands(kos, right_hand_ids, 0)
    await move_to_target(kos, -20.0, 0.0, ACCEL, V_MAX, dt, 21)
    await move_to_target(kos, -70.0, 0.0, ACCEL, V_MAX, dt, 22)
